...
void InaBridge219::begin(int pinSda, int pinScl, uint32_t i2cHz) {
...
float InaBridge219::readBusVoltage() {
  const uint16_t raw = readU16(REG_BUS_V);
  return (float)((raw >> 3) * 4) / 1000.0f;
}

float InaBridge219::readShuntVoltage() {
  const int16_t raw = readS16(REG_SHUNT_V);
  return (float)raw * 10.0e-6f;
}

float InaBridge219::readCurrent() {
  const int16_t raw = readS16(REG_CURRENT);
  return (float)raw * (_imaxA / 32768.0f);
}

float InaBridge219::readPower() {
  const uint16_t raw = readU16(REG_POWER);
  return (float)raw * 20.0f * (_imaxA / 32768.0f);
}

bool InaBridge219::dataReady() {
  return (readU16(REG_BUS_V) & 0x0002) != 0;
}

// ── Configuration API ────────────────────────────────────────────

void InaBridge219::startStreaming() { _streaming = true; }
void InaBridge219::stopStreaming()  { _streaming = false; }

void InaBridge219::setSampleRate(int hz) {
  _sampleHz = InaJsonl::clampStreamRateI2c(hz);
}

void InaBridge219::setRshunt(float ohm) {
  if (ohm <= 0.0f) ohm = 0.1f;
  _rshuntOhm = ohm;
  applyCalibration();
}

void InaBridge219::setImax(float ampere) {
...
void InaBridge219::emitJsonSample() {
...
void InaBridge219::handleCommand(const String& line) {
  ...
  if (cmd.startsWith("ALERT ")) {
    ...
    if (rest.equalsIgnoreCase("CNVR")) {
      ...
      writeU16(REG_ALERT_LIMIT, Ina::Ina219Driver::busVoltageToAlertLimitRaw(v));
      me = 0x2000;
      ...
      writeU16(REG_ALERT_LIMIT, Ina::Ina219Driver::shuntVoltageToAlertLimitRaw_uV(uV));
      me = 0x8000;
    } else {
