...
| Item | URL / note |
|------|------------|
| **This library (source + Arduino Library Manager homepage)** | **`https://github.com/NiusRobotLab/INA_series_sensor`** — set as **`url=`** in **`library.properties`** so the Arduino Library Manager index can list releases and updates. |
...
2. Keep **`url=`** in **`library.properties`** equal to the **public repository home page** (default: **`https://github.com/NiusRobotLab/INA_series_sensor`**). The indexer uses this URL to discover new **tags/releases**.
3. Create a **Git tag** (or GitHub Release) on a commit where `version` in `library.properties` matches the release (e.g. tag **`v0.2.1`** for version **`0.2.1`**).
...
If Git is already initialized (with commit and tag **`v0.2.1`**), add the remote and push:

```bash
git remote add origin https://github.com/NiusRobotLab/INA_series_sensor.git
