/**
 * INA4235 – ALERT interrupt placeholder.
 *
 * =========================================================================
 * Module compatibility
 * =========================================================================
 * The INA4235 is a 4-channel power monitor (1.7–5.5 V supply, up to 48 V
 ...
 * =========================================================================
 *   INA4235 VCC     ->  1.7–5.5 V
 *   INA4235 GND     ->  GND
 *   INA4235 SDA     ->  GPIO 8  (I2C SDA)
 *   INA4235 SCL     ->  GPIO 9  (I2C SCL)
 *   INA4235 ALERT   ->  GPIO 2  (open-drain, add 10 k pull-up to VCC)
 *   INA4235 EN      ->  VCC  (or GPIO for on/off control)
 *   INA4235 CBYPASS ->  0.1 uF cap to GND
 *   INA4235 INP1/INM1 .. INP4/INM4 -> per-channel shunt inputs
 *
 * =========================================================================
 ...
 * INA4235.  To actually configure an alert threshold, you must write the
...
static InaBridgeCh1 sensor("INA4235", "INA4235 bridge ready (CH1)", 0x40);
...
void setup() {
  ...
  Serial.print(F("{\"v\":1,\"type\":\"ALERT_CFG\",\"chip\":\"INA4235\",\"addr\":\"0x40\""));
  Serial.print(F(",\"mode\":\"NONE\",\"pin\":"));
  Serial.print(ALERT_PIN);
  Serial.println(F(",\"_note\":\"Alert API not yet implemented for INA4235; configure via raw register writes per datasheet\"}"));
}
...
void loop() {
  ...
  if (alertTriggered) {
    ...
    Serial.print(F("{\"v\":1,\"type\":\"ALERT_EVENT\",\"chip\":\"INA4235\",\"addr\":\"0x40\""));
    Serial.print(F(",\"t_ms\":"));  Serial.print(millis());
    Serial.println(F(",\"_note\":\"Raw alert detected; decode per INA4235 datasheet\"}"));
  }
}
