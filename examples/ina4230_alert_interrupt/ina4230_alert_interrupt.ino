/**
 * INA4230 – ALERT interrupt placeholder.
 *
 * =========================================================================
 * Module compatibility
 * =========================================================================
 * The INA4230 is a 4-channel power monitor (1.7–5.5 V supply, up to 48 V
 ...
 * =========================================================================
 *   INA4230 VCC     ->  1.7–5.5 V
 *   INA4230 GND     ->  GND
 *   INA4230 SDA     ->  GPIO 8  (I2C SDA)
 *   INA4230 SCL     ->  GPIO 9  (I2C SCL)
 *   INA4230 ALERT   ->  GPIO 2  (open-drain, add 10 k pull-up to VCC)
 *   INA4230 EN      ->  VCC  (or GPIO for on/off control)
 *   INA4230 CBYPASS ->  0.1 uF cap to GND
 *   INA4230 INP1/INM1 .. INP4/INM4 -> per-channel shunt inputs
 *
 * =========================================================================
 ...
 * INA4230.  To actually configure an alert threshold, you must write the
...
void setup() {
  ...
  Serial.print(F("{\"v\":1,\"type\":\"ALERT_CFG\",\"chip\":\"INA4230\",\"addr\":\"0x40\""));
  Serial.print(F(",\"mode\":\"NONE\",\"pin\":"));
  Serial.print(ALERT_PIN);
  Serial.println(F(",\"_note\":\"Alert API not yet implemented for INA4230; configure via raw register writes per datasheet\"}"));
}
...
void loop() {
  ...
  if (alertTriggered) {
    ...
    Serial.print(F("{\"v\":1,\"type\":\"ALERT_EVENT\",\"chip\":\"INA4230\",\"addr\":\"0x40\""));
    Serial.print(F(",\"t_ms\":"));  Serial.print(millis());
    Serial.println(F(",\"_note\":\"Raw alert detected; decode per INA4230 datasheet\"}"));
  }
}
