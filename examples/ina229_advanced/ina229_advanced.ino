/**
 * INA229 – Advanced demo (SPI): temperature + energy/charge + alert.
 *
 * =========================================================================
 ...
 * The INA229 has an integrated die temperature sensor and 40-bit
 ...
 *   INA229 MISO   →  GPIO 5
 *   INA229 MOSI   →  GPIO 6
 *   INA229 ALERT  →  GPIO 2  (open-drain, 10 kΩ pull-up)
 *   INA229 VS     →  3.3 V or 5 V
 *   INA229 GND    →  GND
 *   INA229 INP    →  load high-side     INM → load positive terminal
 *
 * =========================================================================
...
void setup() {
  ...
  Serial.print(F("{\"v\":1,\"type\":\"ALERT_CFG\",\"chip\":\"INA229\",\"addr\":\"SPI\""));
...
void loop() {
  ...
  if (alertTriggered) {
    ...
    Serial.print(F("{\"v\":1,\"type\":\"ALERT_EVENT\",\"chip\":\"INA229\",\"addr\":\"SPI\""));
  ...
  if ((now - g_lastEnergyMs) >= ENERGY_REPORT_MS) {
    ...
    if (clsb > 0.0f) {
      ...
      Serial.print(F("{\"v\":1,\"type\":\"ENERGY\",\"chip\":\"INA229\",\"addr\":\"SPI\""));
